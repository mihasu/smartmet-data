...
curl -s $STATION_URL | jq -r '.[] | [.longitude, .latitude, .siteID] | @csv' | sed 's/"//g' | awk '{print NR "," NR "," $0}' > $STATION_FILE

echo "Downloading site data for each station..."
tail -n +2 $STATION_FILE | while IFS=, read -r stationID stationN longitude latitude siteID; do
...
echo $QD_FILE
csv2qd -v -S ${STATION_FILE} --order=idtime -p Temperature,Humidity,Pressure,PrecipitationAmount,WindDirection,WindSpeedMS -i ${WORKING_FILE} -o ${QD_FILE}
